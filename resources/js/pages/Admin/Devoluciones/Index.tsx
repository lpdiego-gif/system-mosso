...
import { CheckCircle2, ChevronLeft, ChevronRight, Clock, Eye, RotateCcw, Search, X } from 'lucide-react';
...
interface PageProps {
    devoluciones: Paginated<DevolucionRow>;
    filtros: Filtros;
    stats: { total: number; pendientes: number; aprobadas: number; completadas: number };
...
export default function Index({ devoluciones, filtros, stats, opciones }: PageProps) {
    ...
    function recargar(overrides: RecargaParams = {}) {
        ...
        router.get(route('admin.devoluciones.index'), params, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
            only: ['devoluciones', 'filtros'],
        });
    }

    function irA(url: string) {
        router.get(url, {}, { preserveState: true, preserveScroll: true, replace: true, only: ['devoluciones', 'filtros'] });
    }

    const filtrado = (filtros.search ?? '') !== '' || filtros.estado !== null || filtros.tipo !== null;
    const sinResultados = devoluciones.data.length === 0;
        ...
        <>
            ...
            <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 p-4 sm:p-6 lg:p-8">
                <div className="flex flex-col gap-4 rounded-xl border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-start gap-3.5">
                        ...
                        <div className="space-y-1">
                            <h1 className="text-xl font-semibold tracking-tight text-foreground sm:text-2xl">Cambios y devoluciones</h1>
                            <p className="max-w-prose text-sm text-pretty text-muted-foreground">
                                Solicitudes registradas por clientes desde el Portal Web.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                    <StatCard title="Solicitudes" value={String(stats.total)} subtitle="En total" icon={RotateCcw} gradient="from-[#3f4146] to-[#232427]" />
                    <StatCard title="Pendientes" value={String(stats.pendientes)} subtitle="Por revisar" icon={Clock} gradient="from-amber-500 to-orange-600" />
                    <StatCard title="Aprobadas" value={String(stats.aprobadas)} subtitle="En trámite" icon={CheckCircle2} gradient="from-sky-500 to-blue-600" />
                    <StatCard title="Completadas" value={String(stats.completadas)} subtitle="Cerradas" icon={CheckCircle2} gradient="from-emerald-500 to-teal-600" />
                </div>
                ...
                <div className="flex flex-col gap-3 rounded-xl border bg-card p-3 shadow-sm sm:flex-row sm:items-center">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            placeholder="Buscar por N° de solicitud, cliente o correo…"
                    ...
                    <select value={filtros.tipo ?? ''} onChange={(e) => recargar({ tipo: e.target.value || null })} className={cn(inputBase, 'sm:w-40')} aria-label="Filtrar por tipo">
                        <option value="">Cambio y devolución</option>
                    ...
                    {sinResultados ? (
                        <div className="flex flex-col items-center justify-center gap-2 px-6 py-16 text-center">
                            <span className="flex size-12 items-center justify-center rounded-2xl bg-mosso-yellow/15 text-mosso-dark ring-1 ring-inset ring-mosso-yellow/30 dark:text-mosso-yellow">
                                {filtrado ? <Search className="size-6" /> : <RotateCcw className="size-6" />}
                            </span>
                            <h3 className="mt-2 text-sm font-semibold text-foreground">{filtrado ? 'Sin coincidencias' : 'Todavía no hay solicitudes'}</h3>
                            <p className="max-w-sm text-sm text-muted-foreground">
                                {filtrado ? 'Ninguna solicitud coincide con el filtro seleccionado.' : 'Cuando un cliente pida un cambio o devolución, aparecerá aquí.'}
                            </p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full min-w-[760px] text-sm">
                                ...
                                <tbody className="divide-y">
                                    {devoluciones.data.map((d) => (
                                        <tr key={d.id_devolucion} className="transition-colors hover:bg-muted/30">
                                            <td className="px-4 py-3 font-medium tabular-nums text-foreground">#{d.id_devolucion}</td>
                                            <td className="px-4 py-3 font-medium text-foreground">{d.cliente}</td>
                                            <td className="px-4 py-3 text-muted-foreground">{d.tipo === 'cambio' ? 'Cambio' : 'Devolución'} · {d.items_count} ítem(s)</td>
                                            <td className="px-4 py-3 tabular-nums text-muted-foreground">#{String(d.fk_pedido).padStart(6, '0')}</td>
                                            <td className="px-4 py-3 text-muted-foreground" title={d.creado_en}>
                                                {tiempoRelativo(d.creado_en)}
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className={cn('inline-block rounded-full px-2.5 py-0.5 text-xs font-medium', ESTADO_TONO[d.estado] ?? 'bg-muted text-muted-foreground')}>
                                                    {ESTADO_LABEL[d.estado] ?? d.estado}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-right">
                                                <Link href={route('admin.devoluciones.show', d.id_devolucion)} className="inline-flex items-center gap-1.5 rounded-md border bg-background px-2.5 py-1.5 text-xs font-medium shadow-xs transition-colors hover:bg-accent">
                ...
                {!sinResultados ? <Pagination data={devoluciones} disabled={cargando} onNavigate={irA} /> : null}
            </div>
        </>
    );
}
...
Index.layout = {
    breadcrumbs: [{ title: 'Cambios y devoluciones', href: '/admin/devoluciones' }],
};

function Pagination({ data, disabled, onNavigate }: { data: Paginated<DevolucionRow>; disabled: boolean; onNavigate: (url: string) => void }) {
