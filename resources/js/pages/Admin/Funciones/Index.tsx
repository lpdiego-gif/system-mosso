...
export default function Index({ items, filtros, opciones }: PageProps) {
    ...
    const [editando, setEditando] = useState<FuncionItem | null>(null);

    const [eliminando, setEliminando] = useState<FuncionItem | null>(null);
    ...
    useEffect(() => {
        ...
        const t = window.setTimeout(() => {
            ...
            router.get(route('admin.funciones.index'), params, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
                only: ['items', 'filtros'],
    ...
    function abrirEdicion(item: FuncionItem) {
        setEditando(item);
        form.clearErrors();
        form.setData({
            clave: item.clave,
    ...
    function enviarFormulario(e: FormEvent) {
        ...
        if (editando) {
            form.put(route('admin.funciones.update', editando.id_funcion), opts);
        } else {
            form.post(route('admin.funciones.store'), opts);
        }
    }

    function alternarEstado(item: FuncionItem) {
        router.patch(
            route('admin.funciones.toggle-status', item.id_funcion),
            {},
            {
                preserveScroll: true,
                preserveState: true,
                onStart: () => setAlternandoId(item.id_funcion),
    ...
    function confirmarEliminacion() {
        ...
        router.delete(route('admin.funciones.destroy', eliminando.id_funcion), {
            preserveScroll: true,
    ...
    return (
        <>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            Interruptores para entregar el proyecto por fases:
                            mientras una función esté apagada, sus páginas no
                            son accesibles y desaparece de los menús.
                        </p>
                    </div>
                    <button
                        type="button"
                    ...
                    >
                        <Plus className="size-4" /> Nueva función
                    </button>
                </div>
                                                ...
                                                >
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-3">
                                                            <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-400 dark:bg-slate-800">
            ...
            <Modal
                open={modalAbierto}
                onClose={cerrarModal}
                title={editando ? 'Editar función' : 'Nueva función'}
                description={
                    editando
                        ? `Actualiza los datos de «${editando.nombre}».`
                        : 'Agrega un nuevo interruptor de módulo.'
                }
                footer={
                    <>
                        <button
            ...
            >
                <p className="text-sm text-slate-600 dark:text-slate-300">
                    ...
                    ? Si algún middleware o menú todavía usa la clave{' '}
                    <span className="font-mono text-xs">
                        {eliminando?.clave}
                    </span>
                    , esa parte quedará habilitada por defecto al no
                    encontrar ninguna fila para esa clave.
                </p>
            </Modal>
        </>
    );
...
Index.layout = {
    breadcrumbs: [{ title: 'Funciones', href: '/admin/funciones' }],
};
...
function RowActions({
    ...
    item: FuncionItem;
    alternando: boolean;
    onEdit: (item: FuncionItem) => void;
    onToggle: (item: FuncionItem) => void;
    onDelete: (item: FuncionItem) => void;
}) {
    return (
        <div className="flex items-center justify-end gap-1">
            ...
            <ActionButton
                title={item.activo ? 'Desactivar función' : 'Activar función'}
    ...
    return (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center dark:border-slate-700 dark:bg-slate-900">
            ...
            <h3 className="mt-4 text-sm font-semibold text-slate-900 dark:text-slate-100">
                {buscando ? 'Sin resultados' : 'Aún no hay funciones'}
            </h3>
            <p className="mt-1 max-w-sm text-sm text-slate-500 dark:text-slate-400">
                {buscando
                    ? `No encontramos funciones que coincidan con «${termino}».`
                    : 'Crea el primer interruptor de módulo.'}
            </p>
            <div className="mt-4">
                {buscando ? (
                    <button
...
function FuncionForm({
    ...
    useEffect(() => {
        const t = window.setTimeout(
            () => (editando ? nombreRef : claveRef).current?.focus(),
            50,
        );
    ...
    const claveBloqueada = editando !== null;

    return (
        <form id="funcion-form" onSubmit={onSubmit} className="space-y-4">
            <Field
                label="Clave"
                required
                error={errors.clave}
                hint={
                    claveBloqueada
                        ? 'La clave no se puede cambiar una vez creada — el código la referencia directamente.'
                        : 'Identificador técnico, sin espacios. Ej. checkout_culqi'
                }
            >
                <input
                    ref={claveRef}
                    ...
                    onChange={(e) => form.setData('clave', e.target.value.trim())}
                    placeholder="ej. checkout_culqi"
            ...
            <Field label="Nombre" required error={errors.nombre}>
                <input
                    ...
                    placeholder="Ej. Checkout con Culqi"
                    className={inputBase}
                />
            </Field>
            ...
            <Field
                ...
                hint="Para recordar a qué parte del sitio afecta"
            >
                <input
                    type="text"
                    maxLength={255}
                    ...
                    placeholder="Ej. Flujo completo de compra en línea"
                    className={inputBase}
                />
            </Field>
            ...
            <div className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
                <ToggleRow
                    label="Activo"
                    description="Habilitado para los clientes"
                    checked={data.activo}
