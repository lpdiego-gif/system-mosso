...
> ### Subsistemas nuevos
>
> **1. Roles y permisos (RBAC real).** Reemplaza el `middleware('auth')` a secas de todo `/admin`.
> - Tablas ahora **con migración Laravel propia** (guardadas con `Schema::hasTable` para ser no-op donde el dump ya las creó): `roles`, `permisos` (`clave` tipo `modulo.accion`), `rol_permisos` (pivote, PK compuesta, `cascadeOnDelete`), `trabajadores`. Se sembraron ~29 permisos y un reparto por defecto por rol (`Administrador` = todo, `Vendedor`/`Almacenero` = subconjunto). Roles reales en `mosso2`: Super Administrador, Administrador, Vendedor, Almacenero, Cliente.
> - `App\Services\PermisoService` (singleton, memoiza por request). El rol **Super Administrador** tiene bypass total (no necesita filas en `rol_permisos`). El rol **Administrador** puede entrar a `/admin/roles` pero con "delegación con techo": sólo toca roles operativos de su equipo (nunca Super Admin / Administrador / Cliente / a sí mismo) y sólo puede **otorgar** permisos que él ya tiene (quitar siempre se permite).
> - `AppServiceProvider` expone `Gate::define('permiso', ...)` + `Gate::before` (bypass Super Admin), así que sirve `$user->can('permiso','productos.crear')` además del middleware.
> - Middleware nuevos (alias en `bootstrap/app.php`): `permiso:<clave>,<redirectA=/dashboard>` (`EnsurePermiso`), `super_admin:<redirectA>` (`EnsureSuperAdmin` — Roles/Menús/Funciones son exclusivos suyos y NO son permisos delegables), `gestiona.roles` (`EnsureGestionaRoles` — Super Admin + Administrador). Todos flashean un `toast` de error y redirigen (no `abort(403)`).
> - `HandleInertiaRequests` comparte `misPermisos` (array de claves — OJO: se llama así, no `permisos`, para no chocar con el prop de página de `Admin/Roles/Index`), `esSuperAdmin`, `puedeGestionarRoles`. El sidebar (`app-sidebar.tsx`) filtra sus ítems operativos contra `misPermisos` y muestra bloques "Equipo" (Roles) / "Super Admin" (Menús, Funciones) por separado.
> - `routes/web.php` **se reescribió entero**: cada grupo de `/admin/*`, `/trabajador/*`, `/distrito/*`, `/empresa/*` y `dashboard` va detrás de su `permiso:<clave>`. `TrabajadorController` y `DistritoController` ya no dependen sólo de `auth`.
>
> **2. Funciones = feature flags (entrega por fases).** Tabla `funciones` (`clave` UNIQUE, `activo` default `false`) + `Funcion` model (invalida caché en `saved`/`deleted`, junto con `MenuService` y `MenuCuentaService`). `App\Services\FuncionService::estados()` (cacheado 5 min, prop compartido `funciones`). Middleware `feature:<clave>,<redirectA=/>` (`EnsureFuncionActiva`): si la clave no tiene fila, se considera **activa** (sólo los módulos que se agregan explícitamente quedan sujetos al interruptor). Estado real en `mosso2`: `servicios=ON`, `mascotas=off`, `puntos_cupones=off`. Pantalla admin: `/admin/funciones` (`super_admin`).
>
> **3. Menú configurable de "Mi Cuenta"** (`menu_cuenta`). Calcado del patrón de `menus`. `tipo` ∈ (`seccion_interna` con `clave` que el frontend mapea a una página real, `url` enlace libre). `App\Services\MenuCuentaService` (prop compartido `menuCuenta`, cacheado): además de `activo` en BD, oculta `mascotas`/`puntos_cupones` si su feature flag está apagado. Middleware `menu.cuenta:<clave>` (`EnsureMenuCuentaActivo`) protege cada sección de `/mi-cuenta/*` (redirige al escritorio si el admin la desactivó). Pantalla admin: `/admin/menu-cuenta` (`super_admin`). Seed: pedidos, direcciones, mascotas, puntos_cupones, detalles.
>
> **4. Middleware `menu.animal:<animal|categoria|subcategoria>`** (`EnsureMenuAnimalActivo`): bloquea el catálogo público de un animal si no tiene ningún ítem activo en `menus` (ni directo `tipo_enlace=animal` ni vía su grupo `tipo_enlace=tipo_animal`). Aplicado a las 3 rutas `/catalogo/{subcategoria|categoria|animal}/{id}`.
>
> ### Features de negocio nuevas (antes 🔴 "sólo esquema")
>
> - **Servicios** (peluquería/veterinaria) — IMPLEMENTADO. Público: `ServicioController` (`/servicios`, `/servicios/{tipo}`, `/servicio/{slug}` detalle) detrás de `feature:servicios`. Admin: `Admin\ServicioController` + `Admin\TipoServicioController` (CRUD, `permiso:servicios.*`). Modelos `Servicio` (+ `slug`, scope `activos()`), `ServicioBeneficio`, `ServicioHorario`, `ServicioImagen`, `ServicioRed`, `TipoServicio`. Páginas `servicios/index.tsx`, `servicios/show.tsx`, `Admin/Servicios/*`.
> - **Cambios y devoluciones** — IMPLEMENTADO. Tabla nueva `devoluciones` + `devolucion_detalle` (migración `2026_09_01_090002`; FK `fk_cliente`/`fk_pedido`/`fk_pedido_detalle` van como `integer()` **con signo** porque así son las PK del esquema de negocio). Público (cliente auth): `DevolucionController` (`/cambios-y-devoluciones`), sólo permite pedir cambio/devolución de pedidos **Entregados** y por la cantidad de cada ítem que no tenga ya una solicitud activa. Admin: `Admin\DevolucionController` (ver + cambiar estado, `permiso:devoluciones.*`). Modelos `Devolucion`, `DevolucionDetalle`. Páginas `cambios-y-devoluciones.tsx`, `Admin/Devoluciones/*`.
> - **Pedidos (admin)** — `Admin\PedidoController` (listado paginado server-side + detalle + cambio de estado, `permiso:pedidos.*`). Eloquent con `whereRaw('CONCAT_WS(...)')` — MySQL-only, sin test.
> - **Libro de reclamaciones (admin)** — `Admin\ReclamoController` (ver + cambiar estado). `reclamos` ganó `estado` (`pendiente`/`en_proceso`/`resuelto`), `nota_admin`, `atendido_en` (migración `2026_09_01_090001`). El formulario público no cambió.
> - **Clientes (admin)** — `Admin\ClienteController` ahora es CRUD completo (antes el ítem del sidebar apuntaba a `/client` roto). `permiso:clientes.*`. Páginas `Admin/Clientes/{Index,Create,Edit,Show}.tsx`.
> - **Empresa** — `EmpresaController` con UI real en `/empresa` (`permiso:empresa.*`): razón social, RUC, logo, contacto, dirección (departamento→provincia→distrito). Al guardar invalida el caché `shared.empresa`. Los placeholders `[Completar ...]` del Libro de Reclamaciones ahora salen de datos reales de `empresa` (prop compartido).
> - **Productos** — ahora CRUD completo: `edit`/`update`/`destroy`/`toggleEstado`/`ajustarStock` + endpoints `categorias`/`subcategorias`/`etapas` para selects encadenados y `buscar-codigo`. Páginas `Admin/Productos/{Index,Create,Edit}.tsx`.
> - **Catálogo público completo con PDF** — `CatalogoPublicoController` (`/catalogo`, `/catalogo/pdf`). Usa `barryvdh/laravel-dompdf` (dep nueva en composer.json), vista `resources/views/pdf/catalogo.blade.php`, redimensiona imágenes con GD a data URI (fallback `file:///`). *Deja un `Log::info` "diagnóstico temporal" para los primeros 5 productos en `resolverImagenPdf()` — pendiente de quitar.* Página `catalogo/publico.tsx`.
> - **Marcas (público)** — `MarcaCatalogoController` (`/marcas` listado, `/marcas/{marca}` detalle con sus productos). Página `marcas/index.tsx`.
> - **Menú "Mi Cuenta": Mascotas y Puntos** — `MiCuentaMascotaController`, `MiCuentaPuntosController` (detrás de `menu.cuenta:*` + `feature:*`). Modelos `Mascota`, `PuntoCliente`, `CuponCliente` (tablas `mascotas`, `puntos_cliente`, `cupones` — migraciones `2026_08_26_*`). Hoy los feature flags están apagados. `MiCuentaPedidosController` (Mis pedidos + cancelar) sí está activo.
>
> ### Gotchas / deuda nueva a tener presente
>
> - **`app.tsx` layout resolver desactualizado (BUG).** `resources/js/pages/marcas/index.tsx` y `resources/js/pages/cambios-y-devoluciones.tsx` **NO están** en el `case` que devuelve `null`, así que caen al `default` (`AppLayout`, sidebar admin) y además renderizan su propio `<StorefrontLayout>` dentro → doble shell. Hay que agregar `case name === 'marcas/index':` y `case name === 'cambios-y-devoluciones':` (o el prefijo `marcas/`) a ese bloque. Es exactamente el gotcha ya documentado en la sección 7.
> - **Los tests siguen sin un `TestCase` base que provea las fixtures de props compartidos.** `HandleInertiaRequests` ahora consulta en CADA respuesta Inertia: `menus`, `empresa`, `menu_cuenta`, `funciones` (+ `rol_permisos`/`permisos`/`roles`/`trabajadores` sólo si el usuario es trabajador). Las 4 nuevas migraciones crean `menu_cuenta`/`funciones`/`roles`/`permisos`/`rol_permisos`/`trabajadores` en sqlite, pero `menus`/`empresa`/`productos`/etc. siguen sin migración, así que cualquier test que haga `->assertOk()` sobre una página necesita crear `menus` y `empresa` a mano en su `setUp()` (`DashboardTest` crea `menus` pero le falta `empresa` → por eso ahora falla). `CheckoutTest` sí pasa completo.
> - **`base de datos/mosso.sql` ahora SÍ está trackeado** (commit `531e3f0`), pero está desactualizado: es el dump de la BD vieja `mosso` (~Aug 27, 60 tablas) y no incluye `funciones`, `menu_cuenta`, `devoluciones`, `devolucion_detalle`, ni las columnas de gestión de `reclamos`. La fuente de verdad es la BD viva `mosso2`.
> - Middleware de invitado: `redirectGuestsTo` ahora también manda a `/cuenta` (login del storefront) para las rutas `devoluciones.*`, no sólo `checkout.*`.
...
- **Catálogo público por subcategoría/categoría/animal** (`/catalogo/subcategoria/{id}`, `/catalogo/categoria/{id}`, `/catalogo/animal/{id}`): `CatalogoController` con 3 métodos + método privado `formato()` usando `Storage::url()`. Página `resources/js/pages/catalogo/index.tsx` con `StorefrontLayout`, breadcrumbs dinámicos, grid de productos, favoritos y "Agregar al carrito". Las URLs las genera `MenuService` y coinciden con las rutas definidas. Agregado al `case null` en `app.tsx` (prefijo `catalogo/`).
  - **Filtros y orden (2026-09-02)**, 100% client-side sobre el array de productos que ya entrega el controlador (sin endpoints JSON nuevos ni paginación server-side): orden (relevancia/precio asc-desc/novedades/mayor descuento), toggle "en oferta", rango de precio (sobre `precioFinal`, no el de lista), marca (multi, checkboxes con contador) y categoría/subcategoría (selección única, anidada, según el nivel — `nivel: 'animal'|'categoria'|'subcategoria'` viaja como prop nuevo). `CatalogoController::formato()` ahora también expone `stock`, `marcaId`, `categoriaId`/`categoriaNombre`, `subcategoriaId`/`subcategoriaNombre` (ya venían eager-loaded, sin queries nuevas) — tipado en `types/producto.ts` como `ProductoCatalogo extends ProductoCard` (no se tocó `ProductoCard` para no romper Home/Ofertas/Marcas/Buscar/Favoritos, que comparten ese shape). Lógica en el hook `resources/js/hooks/use-catalogo-filtros.ts` + componente `resources/js/components/catalogo/panel-filtros.tsx`. El estado de filtros sincroniza con el querystring vía `history.pushState`/`popstate` (no `router.get` de Inertia — evita un round-trip al servidor por cada click; el filtrado ya es puro client-side). "Novedades" ordena por `id_producto` DESC como proxy: el payload no expone `created_at` aunque la columna sí exista en la tabla. Cualquier cambio futuro a `formato()` debe ir seguido de `App\Support\CatalogoCache::flush()` (se corrió una vez al agregar estos campos, para no dejar cacheado el shape viejo hasta que expire el TTL de 5 min).
